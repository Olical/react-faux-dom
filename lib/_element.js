...
function element () {
  ...
  Element.DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC = 32
  ...
  Element.prototype.cloneNode = function (deep) {
    ...
    for (var m in this.style) {
      if (this.style.hasOwnProperty(m) && [
        'setProperty',
        'getProperty',
        'getPropertyValue',
        'removeProperty'
      ].indexOf(m) === -1) {
        el.style[m] = this.style[m]
      }
    }
    if (deep) {
  ...
  Element.prototype.compareDocumentPosition = function (other) {
        ...
        return result !== false
          ...
          : node === nodeOne
            ...
            : node === nodeTwo
              ? nodeTwo
              : node.childNodes
                ? getFirstNodeByOrder(node.childNodes, nodeOne, nodeTwo)
                : false
      }, false)
    }

    function isSourceContainTarget (source, target) {
    ...
    function hasContaining (node, other) {
      return isSourceContainTarget(node, other)
        ? Element.DOCUMENT_POSITION_CONTAINED_BY + Element.DOCUMENT_POSITION_FOLLOWING
        : isSourceContainTarget(other, node)
    ...
    var result = hasContaining(this, other)
    if (result) {
      return result
    }
