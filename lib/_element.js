  ...
  Element.prototype.compareDocumentPosition = function (other) {
      ...
      return nodes.reduce(function (result, node) {
        if (result) {
          ...
          if (result) {
            ...
            return recursivelyWalk(node.childNodes, cb)
          }
        }
      }, false)
    }
    ...
    function simpleCompare (node, other) {
      if (node.nodeType === Element.ELEMENT_NODE) {
        while (other.parentNode) {
          other = other.parentNode
          if (other === node) {
            return Element.DOCUMENT_POSITION_CONTAINED_BY +
                Element.DOCUMENT_POSITION_FOLLOWING
          }
        }
      } else if (other.nodeType === Element.ELEMENT_NODE) {
        while (node.parentNode) {
          node = node.parentNode
          if (other === node) {
            return Element.DOCUMENT_POSITION_CONTAINS +
    ...
    function identifyWhichIsFirst (node) {
      return node === other ? 'other'
        : node === reference ? 'reference'
        : undefined
    }

    var reference = this
    var referenceTop = this
    var otherTop = other
    ...
    var result = simpleCompare(this, other)
    if (result) {
      return result
    }
